...
import Attack
from encrypted_CKKS import en_predict, en_FedAvg
from sklearn.cluster import DBSCAN
from Models import ResNet18, Mnist_CNN, LinearNet1, LinearNet2
...
def Feature_extraction_model(Central_par, dev):
    ...
    # FC['conv2.weight'], Std['conv2.weight'] = train_Linear(kernel2_train,20*10*5*5,dev)
...
def train_Linear(weight_train,dimen,dev):
    ...
    if dimen < 1000:
        test_model = LinearNet1(dimen)
        dense = 3
    else:
        test_model = LinearNet2(dimen)
        dense = 4
    ...
    en_weight = en_predict(weight_train.cpu(), test_model, dense)
    print("Server dataset parameters training output: {}".format(en_weight))
    std = en_weight.mean()
    dis = en_weight.max() - en_weight.min()
...
def neural_network_feature_extraction(Upload_Parameters, FC, Std, Dis, dev):
    ...
    predict = np.zeros([args['num_of_clients'],2])

    # predict[:,0] = FC['conv1.weight'](kernel1.view(args['num_of_clients'],-1)).cpu().detach().numpy().reshape(args['num_of_clients'],)
    # predict[:,1] = FC['conv2.weight'](kernel2.view(args['num_of_clients'],-1)).cpu().detach().numpy().reshape(args['num_of_clients'],)
    # predict[:,2] = FC['fc.weight'](weight3.view(args['num_of_clients'],-1)).cpu().detach().numpy().reshape(args['num_of_clients'],)
    # print("plaintext output: \n{}".format(predict))
    
    predict[:,0] = en_predict(kernel1,  FC['conv1.weight'] , 3)
    # predict[:,1] = en_predict(kernel2,  FC['conv2.weight'] , 4)
    predict[:,1] = en_predict(weight3,  FC['fc.weight'] , 4)
    print("After decryption of ciphertext operations: \n{}".format(predict))
    ...
    db = DBSCAN(eps=eps, min_samples=3).fit( predict )
        ...
        if label != -1:
            for client in range(args['num_of_clients']):
                if label_list[client] == label:
                    temp = np.concatenate((temp,predict[client,:].reshape(1,2)),axis=0)
...
parser.add_argument('-by', '--byzantine_size', type=int, default=2, help='number of the byzantine clients')
